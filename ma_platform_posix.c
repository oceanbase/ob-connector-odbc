...
const char* GetDefaultLogDir()
{
  ...
  tmp = getenv("ODBC_LOG_ENABLE");
  if (tmp) {
    _snprintf(LogEnable, sizeof(LogEnable), "%s", tmp);
  }
  
  tmp = getenv("ODBC_LOG_DIR");
  if (tmp) 
  {
    DefaultLogDir= tmp;
  }

  _snprintf(LogFile, sizeof(LogFile), "%s/obodbc.log", DefaultLogDir);
...
{
  char *AscStr;
  size_t AscLen= PtrLength, PtrOctetLen=0;
  BOOL dummyError= 0;
  ...
  else
  {
	...
	/*SqlwcsOctetLen libobclient return error lenth, PtrOctetLen>input len keep intput len*/
    PtrOctetLen= PtrOctetLen > PtrLength*sizeof(SQLWCHAR) ? PtrLength*sizeof(SQLWCHAR):PtrOctetLen;
