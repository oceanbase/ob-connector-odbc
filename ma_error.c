...
SQLRETURN MADB_SetNativeError(MADB_Error *Error, SQLSMALLINT HandleType, void *Ptr)
{
  ...
  if (NativeError == 1722 && strcmp(Sqlstate, "42000") == 0)
  {
    Sqlstate= "22018";
  }
                            ...
                            SQLSMALLINT *StringLengthPtr, my_bool isWChar)
{
  ...
  if (Dbc)
  {
    MDBUG_C_ENTER(Dbc, "MADB_GetDiagField");
  } 
  else if (Stmt)
  {
    MDBUG_C_ENTER(Stmt->Connection, "MADB_GetDiagField");
  }
  else if (Desc)
  {
    MDBUG_C_ENTER(Desc->Dbc, "MADB_GetDiagField");
  }
