...
void MADB_DeleteQuery(MADB_QUERY *Query)
{
  ...
  MADB_DeleteDynamic(&Query->ParamPos);
...
enum enum_madb_query_type MADB_GetQueryType(const char *Token1, const char *Token2)
{
  ...
  if (_strnicmp(Token1, "DECLARE", 7) == 0) /*&& _strnicmp(Token2, "NOT", 3) == 0*/
  {
    return MADB_NOT_ATOMIC_BLOCK;
  }
...
#define SAVE_PARAMPOS(PTR2SAVE) do { Offset= (unsigned int)(PTR2SAVE - Query->RefinedText);\
MADB_InsertDynamic(&Query->ParamPos, (char*)&Offset); } while(0)
...
int ParseQuery(MADB_QUERY *Query)
{
  ...
  MADB_InitDynamicArray(&Query->ParamPos, (unsigned int)sizeof(unsigned int), (unsigned int)MAX(Length / 32, 20), (unsigned int)MAX(Length / 20, 40));
  ...
  while (p < end)
  {
    if (ReadingToken == FALSE)
    {
      ...
      case ':':
      {
        if (Query->QueryType != MADB_QUERY_CREATE_PROC)
        {
