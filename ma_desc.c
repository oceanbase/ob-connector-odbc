...
MADB_Desc *MADB_DescInit(MADB_Dbc *Dbc,enum enum_madb_desc_type DescType, my_bool isExternal)
{
  ...
  memset(Desc, 0, sizeof(MADB_Desc));
...
MADB_SetIrdRecord(MADB_Stmt *Stmt, MADB_DescRecord *Record, MYSQL_FIELD *Field)
{
  ...
  MADB_RESET(Record->BaseCatalogName, Field->db);
  Record->FieldFlags = Field->flags;
  ...
  */
  ...
  Record->DisplaySize= MADB_GetDisplaySize(Field, FieldCs, Stmt->Connection->OracleMode);
...
void MADB_FixDisplaySize(MADB_DescRecord *Record, const MY_CHARSET_INFO *charset, BOOL IsOracle)
{
  switch (Record->ConciseType) {
  case SQL_BIT:
...
void MADB_FixDataSize(MADB_DescRecord *Record, const MY_CHARSET_INFO *charset, BOOL IsOracle)
{
  Record->Length= MADB_GetDataSize(Record->ConciseType, Record->OctetLength, Record->Unsigned == TRUE, Record->Precision, Record->Scale, charset->mbmaxlen, IsOracle);
}
/* }}} */
...
MADB_FixIrdRecord(MADB_Stmt *Stmt, MADB_DescRecord *Record)
{
  ...
  */
  ...
  MADB_FixDisplaySize(Record, &cs, Stmt->Connection->OracleMode);
  MADB_FixDataSize(Record, &cs, Stmt->Connection->OracleMode);
                            ...
                            my_bool isWChar)
{
  ...
  if (Desc)
  {
    MDBUG_C_PRINT(Desc->Dbc, "--MADB_DescGetField%s", "");
  }
...
SQLRETURN MADB_DescCopyDesc(MADB_Desc *SrcDesc, MADB_Desc *DestDesc)
{
  ...
  MDBUG_C_PRINT(SrcDesc->Dbc, "--MADB_DescCopyDesc%s", "");
    ...
    BOOL isWChar)
{
  ...
  if (!(Record= MADB_DescGetInternalRecord(Desc, RecNumber-1, MADB_DESC_READ)))
  {
    MADB_SetError(&Desc->Error, MADB_ERR_07009, NULL, 0);
...
SQLRETURN MADB_DescSetRec(MADB_Desc *Desc,
  SQLSMALLINT RecNumber,
  SQLSMALLINT Type,
  SQLSMALLINT SubType,
  SQLLEN Length,
  ...
  BOOL isWChar)
{
  ...
  if (SQL_DATETIME == Type &&
    (SubType != SQL_CODE_TIMESTAMP || SubType != SQL_CODE_DATE || SubType != SQL_CODE_TIME)) {
    MADB_SetError(&Desc->Error, MADB_ERR_HY000, "Invalid Sub Type Code", 0);
    return Desc->Error.ReturnValue;
  }

  if ((SQL_INTEGER == Type || SQL_SMALLINT == Type) && (Precision != 0 || Scale != 0)){
    MADB_SetError(&Desc->Error, MADB_ERR_HY104, NULL, 0);
    return Desc->Error.ReturnValue;
  }
  ...
  default:
    {
      MADB_SetError(&Desc->Error, MADB_ERR_HY004, NULL, 0);
      return Desc->Error.ReturnValue;
    }
  }
  ...
  /* SQL_DESC_OCTET_LENGTH */
  Record->OctetLength = Length;//MADB_GetTypeLength(Type, Length, Stmt->Connection->OracleMode);
  ...
  if (Record && (Record->DataPtr != NULL || Record->OctetLengthPtr != NULL || Record->IndicatorPtr != NULL))
    Record->inUse = 1;
