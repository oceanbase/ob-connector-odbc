...
typedef enum {SYNC_CMD = 0, ASYNC_CMD} ODBC_CMD_TYPE;
SQLRETURN SQLEndTranInner( SQLSMALLINT handle_type,
        SQLHANDLE handle,
        SQLSMALLINT completion_type,
        ODBC_CMD_TYPE odbc_cmd_type,
        int* status)
{
        ...
        if ( environment -> state == STATE_E2 )
        {
            ...
            while( connection )
            {
                if ( connection -> environment == environment &&
                        connection -> state > STATE_C4 )
                {
                    if ((odbc_cmd_type == SYNC_CMD) && CHECK_SQLENDTRAN( connection ))
                    {
                    ...
                    else if ((odbc_cmd_type == ASYNC_CMD) && CHECK_SQLENDTRAN_ASYNC(connection))
                    {
                        ret = SQLENDTRANASYNC( connection,
                            SQL_HANDLE_DBC,
                            connection -> driver_dbc,
                            completion_type,
                            status);
...
SQLRETURN SQLEndTran( SQLSMALLINT handle_type,
        ...
        SQLSMALLINT completion_type)
{
    return SQLEndTranInner(handle_type, handle, completion_type, SYNC_CMD, NULL);
}

SQLRETURN SQLEndTranAsync( SQLSMALLINT handle_type,
        SQLHANDLE handle,
        SQLSMALLINT completion_type,
        int* status)
{
    return SQLEndTranInner(handle_type, handle, completion_type, ASYNC_CMD, status);
}