...
\n\t\t\tStatement = %p",
    ...
    ret = function_return( SQL_HANDLE_STMT, statement, ret, DEFER_R1 );
    return ret;
}
...
\n\t\t\tStatement = %p",
    ...
    if ( !CHECK_SQLEXECUTE_ASYNC( statement -> connection))
    {
        dm_log_write( __FILE__,
                      __LINE__,
                      LOG_INFO,
    ...
    ret = SQLEXECUTEASYNC( statement -> connection ,
                           statement -> driver_stmt, status);
...
SQLRETURN  SQLGetAsyncConnFd(SQLHDBC connection_handle, int* fd) {
...
\n\t\t\tConnection = %p",
    ...
    if ( !CHECK_SQLGETASYNCCONNFD(connection))
    {
        dm_log_write( __FILE__,
                      __LINE__,
                      LOG_INFO,
    ...
    ret = SQLGETASYNCCONNFD(connection ,
                            connection -> driver_dbc, fd);
    ...
    ret = function_return( SQL_HANDLE_DBC, connection, ret, DEFER_R1);
    return ret;
}