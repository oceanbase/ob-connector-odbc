  ...
  case SQL_ATTR_CURRENT_CATALOG:
    {
      ...
      //oracle mode don't support mysql_select_db
      if (Dbc->mariadb && !Dbc->mariadb->oracle_mode){
        if (mysql_select_db(Dbc->mariadb, Dbc->CatalogName)) {
          return MADB_SetError(&Dbc->Error, MADB_ERR_HY001, mysql_error(Dbc->mariadb), mysql_errno(Dbc->mariadb));
        }
      }
    }
  ...
  case SQL_ATTR_TXN_ISOLATION:
    if (Dbc->mariadb)
    {
      ...
      for (i=0; i < 4; i++)
      {
        ...
        {
          char StmtStr[128];
          if (Dbc->mariadb->oracle_mode){
            _snprintf(StmtStr, sizeof(StmtStr), "SET @@tx_isolation= '%s'",
              MADB_IsolationLevel[i].StrIsolation);
          } else {
            _snprintf(StmtStr, sizeof(StmtStr), "SET SESSION TRANSACTION ISOLATION LEVEL %s",
              MADB_IsolationLevel[i].StrIsolation_mysql);
          }
          LOCK_MARIADB(Dbc);
    ...
    if (!Dbc->TxnIsolation)
    {
      *(SQLULEN *)ValuePtr= SQL_TRANSACTION_REPEATABLE_READ;
      if (Dbc->mariadb)
      {
        MYSQL_RES *result;
        MYSQL_ROW row;
        char *StmtString = NULL;
        if (Dbc->mariadb->oracle_mode){
          StmtString = "SELECT @@tx_isolation as VARIABLE_VALUE from dual";
        } else {
          StmtString = "SELECT VARIABLE_VALUE FROM INFORMATION_SCHEMA.SESSION_VARIABLES WHERE VARIABLE_NAME='TX_ISOLATION'";
        }
                                ...
                                SQLSMALLINT *StringLengthPtr, my_bool isWChar) 
{
  ...
  if (Connection->mariadb->oracle_mode){
    sql = (char*)"SELECT (SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA')) FROM DUAL";
  } else {
    sql = (char *)"SELECT IF(DATABASE() IS NOT NULL,DATABASE(),'null')";
  }
  ...
  if (!SQL_SUCCEEDED(Stmt->Methods->ExecDirect(Stmt, sql, SQL_NTS)) ||
    ...
    MADB_Dsn *Dsn)
{
  ...
  */
  ...
  /*first SQLSetConnectAttr, second ini*/
  if (Connection->LoginTimeout > 0){
    ...
    if (Dsn->ConnectionTimeout > 0){
      ...
      Connection->LoginTimeout = Dsn->ConnectionTimeout;
    }
  }
  ...
  /* set default catalog */
  if (Connection->CatalogName && Connection->CatalogName[0] && !Connection->mariadb->oracle_mode)
  {
  ...
  if (Connection->IsolationLevel)
    for (i=0; i < 4; i++)
    {
      ...
      {
        if (Connection->mariadb->oracle_mode){
          _snprintf(StmtStr, 128, "SET @@tx_isolation= '%s'",
            MADB_IsolationLevel[i].StrIsolation);
        } else {
                          ...
                          SQLSMALLINT BufferLength, SQLSMALLINT *StringLengthPtr, my_bool isWChar)
{
  ...
  case SQL_CONVERT_VARCHAR:
    if (Dbc && Dbc->OracleMode) {
      MADB_SET_NUM_VAL(SQLUINTEGER, InfoValuePtr, MADB_SUPPORTED_CONVERSIONS_ORACLE_VARCHAR, StringLengthPtr);
    } else {
      MADB_SET_NUM_VAL(SQLUINTEGER, InfoValuePtr, MADB_SUPPORTED_CONVERSIONS, StringLengthPtr);
    }
    break;
  ...
  case SQL_IDENTIFIER_QUOTE_CHAR:
    SLen= (SQLSMALLINT)MADB_SetString(isWChar ? &Dbc->Charset : NULL, (void *)InfoValuePtr, BUFFER_CHAR_LEN(BufferLength, isWChar), 
      Dbc->mariadb->oracle_mode?"\"":(MADB_SqlMode(Dbc, MADB_ANSI_QUOTES) ? "\"" : "`"), SQL_NTS, &Dbc->Error);
    break;
...
void GetDsnStr(MADB_Dbc *Dbc, MADB_Dsn *Dsn, MADB_DynString* Str, my_bool isCanConnect)
{
  if (isCanConnect){
    if (Dsn->DSNName) {
      MADB_DynstrAppend(Str, "DSN=");
      MADB_DynstrAppend(Str, Dsn->DSNName);
      MADB_DynstrAppend(Str, ";");
    }
    if (Dsn->UserName) {
      MADB_DynstrAppend(Str, "UID=");
      MADB_DynstrAppend(Str, Dsn->UserName);
      MADB_DynstrAppend(Str, ";");
    }
    if (Dsn->Password) {
      MADB_DynstrAppend(Str, "PWD=");
      MADB_DynstrAppend(Str, Dsn->Password);
      MADB_DynstrAppend(Str, ";");
    }
    if (Dsn->ServerName && Dsn->Port > 0 && Dsn->Catalog) {
      char buf[20] = { 0 };
      sprintf(buf, "%d", Dsn->Port);
      MADB_DynstrAppend(Str, "DBQ=");
      MADB_DynstrAppend(Str, Dsn->ServerName);
      MADB_DynstrAppend(Str, ":");
      MADB_DynstrAppend(Str, buf);
      MADB_DynstrAppend(Str, "/");
      MADB_DynstrAppend(Str, Dsn->Catalog);
      MADB_DynstrAppend(Str, ";");
    }
  } else {
    if (!Dsn->DSNName) {
      MADB_DynstrAppend(Str, "*DSN:DSN=?;");
    }
    if (!Dsn->UserName) {
      MADB_DynstrAppend(Str, "*UID:UID=?;");
    }
    if (!Dsn->Password) {
      MADB_DynstrAppend(Str, "*PWD:PWD=?;");
    }
    if (!Dsn->ServerName || Dsn->Port <= 0 || !Dsn->Catalog) {
      MADB_DynstrAppend(Str, "*DBQ:DBQ=?;");
    }
  }
}

my_bool ParseDBQ(char* DsnStr, char* server, int *port, char *database)
{
#define TRIM_SPACE(p) while(iswspace(*p))p++;
  my_bool isParseOK = FALSE;
  int len = strlen(DsnStr);
  char *start, *p;
  char *buffer = MADB_ALLOC(len + 1);
  memset(buffer, 0, len + 1);
  memcpy(buffer, DsnStr, len);
  buffer[len] = ';';
  start = buffer;
  p = buffer;
...
SQLRETURN MADB_BrowseConnect(MADB_Dbc *Dbc, SQLCHAR *InConnectionString, SQLSMALLINT StringLength1,
  SQLCHAR *OutConnectionString, SQLSMALLINT BufferLength, SQLSMALLINT *StringLength2Ptr, my_bool isWChar)
{
#define INIT_DYNAMIC_STRING 2048
  //DSN=oracle;UID=test;PWD=test;DBQ=xxx.xxx.xxx.xxx:1521/XE;
  MADB_Dsn   *Dsn;
  SQLRETURN   ret = SQL_SUCCESS;
  MADB_DynString Str;
  my_bool     isCanConnect = TRUE;
  char server[INIT_DYNAMIC_STRING] = { 0 };
  char database[INIT_DYNAMIC_STRING] = { 0 };
  int port = 0;
  my_bool bParseDBQ = FALSE;
  SQLSMALLINT len = 0;
  ...
  MADB_InitDynamicString(&Str, "", INIT_DYNAMIC_STRING, INIT_DYNAMIC_STRING);

  Dsn = MADB_DSN_Init();
  if (!MADB_ParseConnString(Dsn, InConnectionString, StringLength1, ';')) {
    goto error;
  }
  if (!Dsn->DSNName || !Dsn->UserName || !Dsn->Password) {
    isCanConnect = FALSE;
  }

  bParseDBQ = ParseDBQ(InConnectionString, server, &port, database);
  if (isCanConnect) {
    if (!MADB_ReadConnString(Dsn, (char *)InConnectionString, StringLength1, ';')) {
      MADB_SetError(&Dbc->Error, MADB_ERR_HY000, "Error while parsing DSN", 0);
      goto error;
    ...
    if (bParseDBQ) {
      ...
      int databaseLen = strlen(database);
      MADB_DSN_SET_STR(Dsn, ServerName, (char *)server, serverLen);
      MADB_DSN_SET_STR(Dsn, Catalog, (char *)database, databaseLen);
      Dsn->Port = port;
    }
    if (!SQL_SUCCEEDED(MADB_DbcConnectDB(Dbc, Dsn))) {
      goto error;
  ...
  GetDsnStr(Dbc, Dsn, &Str, isCanConnect);
  
  if (isWChar) {
    len = (SQLSMALLINT)MADB_SetString(isWChar ? &Dbc->Charset : 0, OutConnectionString, BufferLength, Str.str, Str.length, &Dbc->Error);
    if (StringLength2Ptr)
      *StringLength2Ptr = len;
    if (Str.length*2 > BufferLength){
      ret = SQL_SUCCESS_WITH_INFO;
    }
  } else {
    len = Str.length > BufferLength ? BufferLength : Str.length;
    memcpy(OutConnectionString, Str.str, len);
    if (StringLength2Ptr) 
      *StringLength2Ptr = len;
  ...
  MADB_DynstrFree(&Str);
  if (!isCanConnect || (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)) {
    MADB_DSN_Free(Dsn);
  }
  if ((ret == SQL_SUCCESS || ret== SQL_SUCCESS_WITH_INFO) && !isCanConnect){
    ret = SQL_NEED_DATA;
  }
