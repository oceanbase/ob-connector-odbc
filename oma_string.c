...
char *MADB_GetTableName(MADB_Stmt *Stmt)
{
 ...
 unsigned  int i= 0;
  ...
  for (i = 0; i < MADB_STMT_COLUMN_COUNT(Stmt); i++)
  {
    ...
    if (RecordField != NULL) {
      TableName = RecordField->BaseTableName;
      break;
    }
  }
...
char *MADB_GetCatalogName(MADB_Stmt *Stmt)
{
  ...
  for (i = 0; i < MADB_STMT_COLUMN_COUNT(Stmt); i++)
  {
    MADB_DescRecord *RecordField = MADB_DescGetInternalRecord(Stmt->Ird, i, MADB_DESC_READ);
    if (RecordField != NULL) {
      CatalogName = RecordField->CatalogName;
      break;
    }
  }
...
my_bool MADB_DynStrUpdateSet(MADB_Stmt *Stmt, MADB_DynString *DynString)
{
  ...
  for (i=0; i < MADB_STMT_COLUMN_COUNT(Stmt); i++)
  {
    ...
    RecordField = MADB_DescGetInternalRecord(Stmt->Ird, i, MADB_DESC_READ);
    if (RecordField==NULL){
      return TRUE;
    }
    ...
    if (IS_ORACLE_MODE(Stmt)){
      if (MADB_DynstrAppend(DynString, RecordField->BaseColumnName))
      {
        MADB_SetError(&Stmt->Error, MADB_ERR_HY001, NULL, 0);
        return TRUE;
      }
    } else {
      if (MADB_DynStrAppendQuoted(DynString, RecordField->BaseColumnName))
      {
        MADB_SetError(&Stmt->Error, MADB_ERR_HY001, NULL, 0);
        return TRUE;
      }
  ...
  if (IgnoredColumns == StmtFieldCount(Stmt))
  {
...
my_bool MADB_DynStrInsertSet(MADB_Stmt *Stmt, MADB_DynString *DynString)
{
  ...
  if (MADB_InitDynamicString(&ColVals, "VALUES (", 32, 32) ||
    MADB_DynstrAppend(DynString, " (")) {
    goto dynerror;
  }
...
my_bool MADB_DynStrGetColumns(MADB_Stmt *Stmt, MADB_DynString *DynString)
{
  ...
  for (i=0; i < mysql_stmt_field_count(Stmt->stmt); i++)
  {
    ...
    if (IS_ORACLE_MODE(Stmt)){
      if (MADB_DynstrAppend(DynString, Stmt->stmt->fields[i].org_name))
      {
        MADB_SetError(&Stmt->Error, MADB_ERR_HY001, NULL, 0);
        return TRUE;
      }
...
my_bool MADB_DynStrGetWhere(MADB_Stmt *Stmt, MADB_DynString *DynString, char *TableName, my_bool ParameterMarkers)
{
  ...
  for (i= 0; i < MADB_STMT_COLUMN_COUNT(Stmt); i++)
  {
    ...
    RecordField = MADB_DescGetInternalRecord(Stmt->Ird, i, MADB_DESC_READ);
    if (RecordField){
      if (RecordField->FieldFlags & PRI_KEY_FLAG)
        PrimaryCount++;
      if (RecordField->FieldFlags & UNIQUE_KEY_FLAG)
        UniqueCount++;
    }
  }
  ...
  //if no primary or unique key is in the cursor, the cursor must contain all columns from table in TableName
  if (!PrimaryCount && !UniqueCount)
  {
    ...
    if (IS_ORACLE_MODE(Stmt)){
      _snprintf(StmtStr, 256, "SELECT * FROM %s where rownum=0", TableName);
    } else {
    ...
    FieldCount = StmtFieldCount(((MADB_Stmt *)CountStmt));
  ...
  for (i= 0; i < MADB_STMT_COLUMN_COUNT(Stmt); i++)
  {
    ...
    RecordField = MADB_DescGetInternalRecord(Stmt->Ird, i, MADB_DESC_READ);
    if (RecordField && (RecordField->FieldFlags & Flag || !Flag))
    {
      if (MADB_DynstrAppend(DynString, " AND ") ||
        MADB_DynstrAppend(DynString, RecordField->BaseColumnName))
          goto memerror;
      if (ParameterMarkers)
      {
        if (MADB_DynstrAppend(DynString, "=?"))
      ...
      { 
        ...
        else
        {
          ...
          if (MADB_DynstrAppend(DynString, "= '"))
            goto memerror;
          if (IS_ORACLE_MODE(Stmt)){
            if (MADB_DynstrAppend(DynString, Column))
              goto memerror;
          } else {
            if(MADB_DynstrAppend(DynString, Escaped))
              goto memerror;
          }
          if (MADB_DynstrAppend(DynString, "'"))
            goto memerror;
  ...
  if (IS_ORACLE_MODE(Stmt)){
    if (MADB_DynstrAppend(DynString, " AND rownum=1"))
      goto memerror;
  } else {
